...
class Genetic:
    ...
    def array_factor_dbi(self, b_, d_, w_, m_=-60):
        s = 0
    ...
    def calculate_fitness(self):
        for Member in self.pop:
            ...
            self.ans_dbi = self.array_factor_dbi(decoded[0], decoded[1], decoded[2])

            hp_bw = []
            i = 249
            while i != 0:
                if self.ans_dbi[i] < -2.99:
                    hp_bw.append([self.ans_dbi[i], (((i + 1) * (2 * np.pi)) / 1000) * (180 / np.pi)])
                    break
                i -= 1
            ...
            for i in range(min_dot, 500):
                if self.ans_dbi[i] > sll[0]:
                    sll = [self.ans_dbi[i], ((i * (2 * np.pi)) / 1000) * (180 / np.pi)]
